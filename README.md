...
Every PERSON is classified by one MEMBERTYPE, and one MEMBER_TYPE may have either zero, one, or many people 

A PERSON can be a WALK_IN. A walk-in data belongs to only one person. 

MEMBERSUB and TRAINER are partial, overlapping specializations. A PERSON is not required to be either subtype and the same PERSON can be both a MEMBERSUB and a TRAINER 
